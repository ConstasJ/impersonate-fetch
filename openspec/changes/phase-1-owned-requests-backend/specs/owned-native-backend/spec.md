...
The system SHALL include tests that compare the source-owned backend against the current closed backend for Phase 1 compatibility.
...
The system SHALL generate scoped platform-specific npm backend packages from compiled artifacts during CI/release flow using artifacts produced by the GitHub Actions Matrix builds.
...
- **WHEN** the matching scoped backend package is unavailable
- **THEN** the loader fails with a clear native asset error rather than silently degrading to a non-impersonating transport
