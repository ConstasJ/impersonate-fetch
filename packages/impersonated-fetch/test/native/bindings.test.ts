...
type EncodedEmitter = EventEmitter & {
  setEncoding(encoding: string): EncodedEmitter;
};

class FakeChildProcess extends EventEmitter {
  readonly methods: string[] = [];
  readonly stdout = createEncodedEmitter();
  readonly stderr = createEncodedEmitter();
  readonly stdin = {
    write: (line: string) => {
...
function createEncodedEmitter(): EncodedEmitter {
  const emitter = new EventEmitter() as EncodedEmitter;
